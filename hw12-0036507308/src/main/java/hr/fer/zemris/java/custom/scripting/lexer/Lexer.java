...
/**
 * Lexer class used for geenrating tokens from given smart script text.
 */
public class Lexer {
	...
	/**
	 * Character array containing document content.
	 */
	...
	/**
	 * Current index in document char array.
	 */
	private int currentIndex;
	
	/**
	 * Constructs a Lexer with given document content.
	 * 
	...
	public Token nextToken() {
		...
		if (state.equals(LexerState.EXTENDED)) {
			...
			if (c == '"') {
				...
				currentIndex++;
				String text = sb.toString().replaceAll("\\\\r", "\r").replaceAll("\\\\t", "\t").replaceAll("\\\\n", "\n");
				return token = new Token (TokenType.STRING, text); //token generiran u stringu
			}
	 ...
	 * 
	 * @param state  state to set a lexer to
	 */
